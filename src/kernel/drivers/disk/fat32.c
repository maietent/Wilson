...
static uint32_t fat_find_free_cluster(void)
{
    uint32_t total_clusters
        = boot_sector.total_sectors_32 / boot_sector.sectors_per_cluster;
    ...
    while (cluster < FAT32_EOC) {
        ...
        for (uint8_t s = 0; s < boot_sector.sectors_per_cluster; s++) {
            ...
            for (uint16_t offset = 0; offset < 512; offset += 32) {
                ...
                if (memcmp(ent->name, fat_name, 11) == 0) {
...
static bool create_dir_entry(uint32_t dir_cluster, const char* name,
...
bool fat32_init(void)
{
    ...
    if (mbr[510] != 0x55 || mbr[511] != 0xAA) {
        klogf(
            "FAT32: Invalid MBR signature (0x%02X%02X)\n", mbr[511], mbr[510]);
        return false;
    }
    ...
    if (memcmp(boot_sector.fs_type, "FAT32   ", 8) != 0
        && memcmp(boot_sector.fs_type, "FAT32", 5) != 0) {
...
bool fat32_open(const char* path, fat32_file_t* file)
{
    ...
    uint32_t search_dir = current_dir_cluster;

    if (!find_dir_entry(search_dir, path, &entry, NULL, NULL))
        return false;
...
bool fat32_create(const char* path, fat32_file_t* file)
{
    ...
    if (!create_dir_entry(
            current_dir_cluster, path, new_cluster, 0, FAT32_ATTR_ARCHIVE))
        return false;
...
int fat32_read(fat32_file_t* file, uint8_t* buffer, uint32_t size)
{
    ...
    while (bytes_read < size && file->current_cluster < FAT32_EOC) {
        ...
        uint32_t sector
            = cluster_to_sector(file->current_cluster) + sector_in_cluster;
...
int fat32_write(fat32_file_t* file, const uint8_t* buffer, uint32_t size)
{
    ...
    while (bytes_written < size) {
        ...
        memcpy(
            sector_buffer + offset_in_sector, buffer + bytes_written, to_copy);
...
bool fat32_close(fat32_file_t* file)
{
    ...
    const char* path = "";
    if (find_dir_entry(
            root_dir_cluster, path, &entry, &entry_sector, &entry_offset)) {
        if (!read_sector(entry_sector, sector_buffer))
            return false;

        fat32_dir_entry_t* ent
            = (fat32_dir_entry_t*)&sector_buffer[entry_offset];
...
bool fat32_delete(const char* path)
{
    ...
    if (!find_dir_entry(
            current_dir_cluster, path, &entry, &entry_sector, &entry_offset))
        return false;
...
bool fat32_list_dir(const char* path,
    void (*callback)(const char* name, uint32_t size, uint8_t attr))
{
    uint32_t cluster = current_dir_cluster;

    if (path && *path) {
        fat32_dir_entry_t entry;
        if (!find_dir_entry(current_dir_cluster, path, &entry, NULL, NULL))
            return false;
...
bool fat32_change_dir(const char* path)
{
    ...
    if (strcmp(path, "..") == 0) {
        fat32_dir_entry_t entry;
        if (find_dir_entry(current_dir_cluster, "..", &entry, NULL, NULL)) {
            uint32_t parent_cluster = ((uint32_t)entry.first_cluster_high << 16) | entry.first_cluster_low;
            if (parent_cluster == 0) {
                current_dir_cluster = root_dir_cluster;
            } else {
